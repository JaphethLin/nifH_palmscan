...
const char *SFasta::GetNextSeq()
	...
	for (;;)
		{
		const char *Seq = GetNextSeqLo();	// 读取 Label 和 序列，返回 Seq
		if (Seq == 0)						// 指针在文件末尾时，Seq==0，返回 0 
			{
			if (m_TooLongCount > 0)
		...
		if (m_SeqLength > opt_maxseqlength && opt_maxseqlength != 0)
			{
			if (m_LongestLength == 0 || m_SeqLength > m_LongestLength)		// 获得最长的序列的长度
				m_LongestLength = m_SeqLength;
			++m_TooLongCount;												// 长度大于 opt_maxseqlength 的序列数量
			continue;
			}
		return Seq;
		}
