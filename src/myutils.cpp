...
void myvstrprintf(string &Str, const char *Format, va_list ArgList)
	{
	char *szStr = GetThreadStr();		// 获取当前线程的缓冲字符区 szStr
	vsnprintf(szStr, MAX_FORMATTED_STRING_LENGTH-1, Format, ArgList);	// 将格式化后的字符串输出到 szStr
	szStr[MAX_FORMATTED_STRING_LENGTH - 1] = '\0';
	Str.assign(szStr);					// 将 szStr 中的字符串内容赋值给 Str
	}
...
// Flag 类型参数读取 ；将传入的 Optname 与 Flag 作比较，找到相同 Flag 则将 opt_flagname optset_flagname 设置为 true，并返回 true
...
// 整数 类型参数读取 ；在 UNS_OPT 参数表中查找 OptName ，将 value 赋值给 opt_argName ， 将 optset_argName 赋值为 true 
...
// 字符串类型参数读取 ；在 STR_OPT 参数表中查找 OptName ，将 string value 赋值给 opt_argName ， 将 optset_argName 赋值为 true 
...
#include "myopts.h"
	...
	for (;;)
		...
		if (Arg.size() > 1 && Arg[0] == '-')	// 参数为 ：-xxx (-后面的字符数>0)
			{
			string LongName = (Arg.size() > 2 && Arg[1] == '-' ? Arg.substr(2) : Arg.substr(1));	// 判断参数是否为 --xxx 形式
			if (LongName == "help")				// 判断是否为 --help
				Help();
			else if (LongName == "version")		// 判断是否为 --version
				{
				Version(stdout);
				printf("\n");
				exit(0);
			...
			// Flag 类型 参数读取
			bool IsFlag = TryFlagOpt(LongName.c_str());		// 查找 flag 类的参数，若找到则将 opt_flagname optset_flagname 赋值为 true
			if (IsFlag)		// 若找到 ，读取下一参数，并直接进入下一轮循环
				{
				++ArgIndex;
				continue;
				}

			// 若存在 --xxx/-xxx 类型的参数头但不是 Flag 类型参数，则读取后面的参数值
			++ArgIndex;
			if (ArgIndex >= ArgCount)
...
// 比较 S 和 T 字符串，T 走到 0 时返回 True，S!=T 时返回 false
...
void Version(FILE *f)
	...
	fprintf(f, "palmscan v%s.%s%s (Modified version for scanning nifH)\n", MY_VERSION, GetPlatform(), Flags);
	}
